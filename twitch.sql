...
--What is the average stream duration across all streamers? LOOK BACK ON THIS
SELECT AVG(avearge_stream_duration) AS avg_stream_duration FROM streamers2024;
...
SELECT type, AVG(CAST(avearge_stream_duration AS decimal(10,2))) AS avg_duration, AVG(CAST(total_views AS bigint)) AS avg_total_views
...
WHERE avearge_stream_duration > (SELECT AVG(avearge_stream_duration) FROM streamers2024)
